...
void back(int index, int currWeight, int currProfit) {
	...
	if (currWeight + ws[index] <= maxxW)
		back(index + 1, currWeight + ws[index], currProfit + vals[index]);
	back(index + 1, currWeight, currProfit);
}
...
int main() {
	...
	std::cout << "DP - Tabu took: " << time_span.count() << " seconds !!!\n";

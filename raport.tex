...
The Knapsack problem has as inputs: \textbf{W} - the maximum weight of the sack, \textbf{N} - the number of objects and the \textbf{objects} themselves that have as proprieties: \textbf{weight and profit}. The program returns the maximum profit that you can get without exceeding the weight limit. In the following demonstration we only use the \textbf{fail()} NP function because it doesn't interrupt the execution of the other threads. We also assume that the assignments to the \textbf{MAX\_PROFIT} variable are \textbf{atomic}.

\begin{lstlisting}[language=Python]
MAX_PROFIT = 0 # the result will be kept here    
def knapsack(W, N, Objects):
    seen = N * [0]
    current_weight = 0
    current_profit = 0
    sack = []
